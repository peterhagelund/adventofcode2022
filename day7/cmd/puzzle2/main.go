...
func main() {
	...
	neededSize := 30000000 - (70000000 - root.totalSize())
	smallestSize := 70000000
	findSmallestPossibleSize(root, neededSize, &smallestSize)
...
func findSmallestPossibleSize(e *entry, neededSize int, smallestSize *int) {
	...
	for _, c := range e.children {
		if c.kind == directory {
			findSmallestPossibleSize(c, neededSize, smallestSize)
		}
	}
}
